  ...
  with canvas(device) as draw:
    ...
    if looper==0:
      ...
      if lora_mote != None:
          quality_signal =  0
          if int(lora_rssi)  >= -23:
              quality_signal = quality_signal + 100
          elif int(lora_rssi) < -23 and int(lora_rssi) >= -47:
              quality_signal = quality_signal + 80
          elif int(lora_rssi) <-47 and int(lora_rssi) >= -70:
              quality_signal = quality_signal + 60
          elif int(lora_rssi) <-70 and int(lora_rssi) >= -94:
              quality_signal = quality_signal + 40
          elif int(lora_rssi) < -94 and int(lora_rssi) >= -105:
              quality_signal = quality_signal + 20
          elif int(lora_rssi) <= -105:
              quality_signal = quality_signal + 10

          ip, stats = lan_info("eth0")
          if ip == "No Interface":
             ip, stats = lan_info("wlan0")

          draw.text((col1, line1),"Dev Eui: %s" % lora_mote, font=font10, fill=255)
          draw.text((col1, line2), ip,  font=font10, fill=255)
          draw.text((col1, line3),"RSSI: %sdBi" % lora_rssi, font=font10, fill=255)
          draw.text((col1, line4),"Signal: %d" % quality_signal, font=font10, fill=255)
